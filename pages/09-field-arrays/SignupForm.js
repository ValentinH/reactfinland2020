      ...
      {({ handleSubmit, values }) => (
        <form onSubmit={handleSubmit}>
          ...
          <FieldArray name="hobbies">
            {({ fields }) => (
              <>
                <button type="button" className="add" onClick={() => fields.push('')}>
                  Add Hobby
                </button>
                {fields.map((name, index) => (
                  <div key={name}>
                    <label name={name}>Hobby #{index}</label>
                    <Field component="input" name={name} type="text" placeholder="Hobby" />
                    <button type="button" className="remove" onClick={() => fields.remove(index)}>
                      Remove
                    </button>
                  </div>
                ))}
